...
@section('content')
    ...
    <script type="text/javascript">
        ...
        function viewCredentials(id) {
            ...
            $.ajax({
                type: "POST",
                url: "{{url('admin/view-api-credentials')}}",
                data: dataString,
                success: function (msg) {
                    $(".loader").hide();
                    if (msg.status == 'success') {
                        $("#credentials_api_id").val(msg.api_id);
                        $("#credentials_api_name").val(msg.api_name);
                        var credentials = msg.credentials;
                        generateFormFromJSON(credentials);
                        document.getElementById('formContainer').innerHTML = generateFormFromJSON(credentials);
                        $("#view-api-credentials").modal('show');
                    } else {
                        swal("Failed", msg.message, "error");
                    }
                }
        ...
        function generateFormFromJSON(jsonData) {
            ...
            for (var key in jsonData) {
                if (typeof jsonData[key] === 'object') {
                    ...
                    formHTML += '<fieldset><legend>' + key + '</legend>';
                    formHTML += generateFormFromJSON(jsonData[key]);
                    formHTML += '</fieldset>';
                } else {
                    formHTML += ' <div class="col-sm-12">';
                    formHTML += ' <div class="form-group">';
                    formHTML += ' <label for="name">' + key + ':</label>';
                    formHTML += ' <input type="text" class="form-control" id="' + key + '" name="' + key + '" value="' + jsonData[key] + '">';
                    formHTML += ' </div>';
                    formHTML += '</div>';
                }
            }
        ...
        function updateCredentials() {
            $("#credentialsBtn").hide();
            $("#credentialsBtn_loader").show();
            var token = $("input[name=_token]").val();
            var api_id = $("#credentials_api_id").val();
            var formContainer = document.getElementById('formContainer');
            var formData = {};
            // Traverse through input fields and create JSON object
            var inputs = formContainer.querySelectorAll('input');
            inputs.forEach(function (input) {
                formData[input.getAttribute('name')] = input.value;
            });
            // Send AJAX POST request to Laravel controller
            $.ajax({
                url: "{{url('admin/update-api-credentials')}}",
                type: 'POST',
                dataType: 'json',
                data: {
            ...
            <div class="card">
                <div class="card-header pb-0">
                    <div class="d-flex justify-content-between">
                        <h4 class="card-title mg-b-2 mt-2">Api Master</h4>
                        ...
                        <table class="@if(Auth::User()->company->table_format == 1)table text-md-nowrap @else display responsive nowrap @endif" id="example1">
                            ...
                            <tr>
                                ...
                                @if(Auth::User()->role_id == 1)
                                    <th class="wd-25p border-bottom-0">Credentials</th>
                                @endif
                            </tr>
                            </thead>
                            <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    {{ $value->api_name }}
                                    <small id="slabHelp" class="form-text text-muted mt-0" style="font-size:70%"> <u><a href="{{url('admin/api-commission/v1/welcome')}}/{{ $value->id }}" target="_blank" class="text-info">Commission Setup</a></u></small>
                                </td>
                                <td>{{ $value->support_number }}</td>
                                <td><a  href="{{url('admin/view-api-provider')}}/{{ $value->id }}" class="btn btn-primary btn-sm">Add Provider</a> </td>
                                <td><a href="{{url('admin/webhook-setting')}}/{{ $value->id }}" class="btn btn-primary btn-sm">Webhook Setting</a> </td>
                                <td><a href="{{url('admin/response-setting')}}/{{ $value->id }}" class="btn btn-primary btn-sm">Add Response</a> </td>
                                ...
                                <td>
                                    ...
                                    @if(Auth::User()->role_id == 1)
                                        <td><button class="btn btn-primary btn-sm" onclick="viewCredentials({{ $value->id }})">Update Credentials</button> </td>
                                    @endif
