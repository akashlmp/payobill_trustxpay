...
function refund_transaction(id) {
        $(".loader").show();
        var token = $("input[name=_token]").val();
        var dataString = 'id=' + id +  '&_token=' + token;
        $.ajax({
            type: "POST",
            url: "{{url('admin/report/v1/view-dmt-transaction-details')}}",
            data: dataString,
            success: function (msg) {
                $(".loader").hide();
                if (msg.status == 'success') {
                    $("#dmt_refund_id").val(msg.details.id);
                    $("#dmt_refund_user").val(msg.details.user);
                    $("#dmt_refund_provider").val(msg.details.provider);
                    $("#dmt_refund_number").val(msg.details.number);
                    // $("#dmt_refund_txnid").val(msg.details.txnid);
                    // $("#dmt_refund_status_id").val(msg.details.status_id);
                    $("#dmt_refund_wallet_type").val(msg.details.wallet_type);
                    $("#view_dmt_refund_model").modal('show');
                }else{
    ...
    function update_dmt_refund() {
        $("#dmt_refund_btn").hide();
        $("#dmt_refund_btn_loader").show();
        var token = $("input[name=_token]").val();
        var id = $("#dmt_refund_id").val();
        var otp = $('#dmt_refund_otp').val();
        var txnid = '';
        var status_id ="";// $("#dmt_refund_status_id").val();
        var wallet_type = $("#dmt_refund_wallet_type").val();
        var dataString = 'id=' + id + '&txnid=' + txnid + '&otp=' + otp + '&status_id=' + status_id + '&wallet_type=' + wallet_type + '&_token=' + token;
        $.ajax({
            type: "POST",
            url: "{{url('admin/report/v1/dmt-update-for-refund')}}",
            data: dataString,
            success: function (msg) {
                $("#dmt_refund_btn").show();
                $("#dmt_refund_btn_loader").hide();
                if (msg.status == 'success') {
                    $("#view_dmt_refund_model").modal('hide');
    ...
    function resendDmtOtp() {
            $("#resendDmtOtpBtn").hide();
            $("#resendDmtOtpBtn_loader").show();
            var token = $("input[name=_token]").val();
            var id = $("#dmt_refund_id").val();
            var dataString = 'id=' + id + '&_token=' + token;
            $.ajax({
                type: "POST",
                url: "{{url('admin/report/v1/refund-dmt-resend-otp')}}",
                data: dataString,
                success: function (msg) {
                    $("#resendDmtOtpBtn").show();
                    $("#resendDmtOtpBtn_loader").hide();
...
<div class="modal  show" id="view_dmt_refund_model"data-toggle="modal">
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content modal-content-demo">
            <div class="modal-header">
                <h6 class="modal-title">Transaction Details</h6><button aria-label="Close" class="close" data-dismiss="modal" type="button"><span aria-hidden="true">×</span></button>
            </div>
            <div class="modal-body">
                <div class="form-body">

                    <input type="hidden" id="dmt_refund_id">
                    ...
                    <div class="row">
                        ...
                        <div class="col-sm-12">
                            <div class="form-group">
                                <label for="name">Type</label>
                                <select class="form-control" id="dmt_refund_wallet_type">
                                    <option value="1">Normal Wallet</option>
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="col-sm-6">
                            <div class="form-group">
                                <label for="name">User Name</label>
                                <input type="text" id="dmt_refund_user" class="form-control" placeholder="User Name" readonly>
                            </div>
                        </div>
                        ...
                        <div class="col-sm-6">
                            <div class="form-group">
                                <label for="name">Provider</label>
                                <input type="text" id="dmt_refund_provider" class="form-control" placeholder="Provider" disabled>
                            </div>
                        </div>
                        ...
                        <div class="col-sm-6">
                            <div class="form-group">
                                <label for="name">Number</label>
                                <input type="text" id="dmt_refund_number" class="form-control" placeholder="Number" disabled>
                            </div>
                        </div>
                        ...
                        <div class="col-sm-6">
                            <div class="form-group">
                                ...
                                <input type="text" id="dmt_refund_otp" class="form-control" placeholder="Otp">
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="modal-footer">
                <button class="btn ripple btn-danger" type="button" id="resendDmtOtpBtn" onclick="resendDmtOtp()">Resend OTP
                </button>
                <button class="btn btn-danger" type="button" id="resendDmtOtpBtn_loader" disabled style="display: none;">
                    <span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span> Loading...
                </button>
                <button class="btn ripple btn-primary" type="button" id="dmt_refund_btn" onclick="update_dmt_refund()">Refund</button>
                <button class="btn btn-primary" type="button"  id="dmt_refund_btn_loader" disabled style="display: none;"><span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span> Loading...</button>
                <button class="btn ripple btn-secondary" data-dismiss="modal" type="button">Close</button>
            </div>
        </div>
    </div>
        ...
        <div class="modal-content">
                        ...
                        @if(Request::segment(2) == 'money-transfer-report')
                            ...
                            <div class="col-sm-6">
                                <div class="form-group">
                                    ...
                                    <input type="text" id="view_remiter_number" class="form-control" placeholder="Remitter Number" disabled>
                                </div>
                            </div>
                            ...
                            <div class="col-sm-6">
                                <div class="form-group">
                                    ...
                                    <input type="text" id="view_remiter_name" class="form-control" placeholder="Remitter Name" disabled>
                                </div>
                            </div>
                        @endif
            ...
            <div class="modal-footer">
                ...
                @elseif(Auth::user()->role_id == 1)
                    @if(hasAdminPermission('admin.transaction.update'))
                    <button class="btn btn-success" id="update_recharge_anchor"><i class="fas fa-edit"></i> Update</button>
                    @endif
                @endif
                    {{--view api logs--}}
