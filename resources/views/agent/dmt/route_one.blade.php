...
@section('content')
    ...
    <script type="text/javascript">
        ...
        function getCustomer() {
            ...
            var dataString = 'mobile_number=' + mobile_number + '&name=' + name + '&latitude=' + latitude + '&longitude=' + longitude + '&_token=' + token;
            $.ajax({
                type: "POST",
        ...
        function confirmSender() {
            ...
            var latitude = $("#inputLatitude").val();
            var longitude = $("#inputLongitude").val();
            var BiometricDataPid = $("#BiometricDataPid").val();
            var dataString = 'mobile_number=' + mobile_number + '&first_name=' + first_name + '&aadhaar_number=' + aadhaar_number + '&last_name=' + last_name + '&pincode=' + pincode + '&state=' + state + '&address=' + address + '&ad1=' + ad1 + '&ad2=' + ad2 + '&latitude=' + latitude + '&longitude=' + longitude + '&_token=' + token + '&BiometricDataPid=' + encodeURIComponent(BiometricDataPid);
            $.ajax({
                type: "POST",
                ...
                success: function (msg) {
                    ...
                    if (msg.status == 'error') {
                        ...
                        $("#state_errors").text(msg.errors.state);
                        $("#address_errors").text(msg.errors.address);
                        $("#aadhaar_number_errors").text(msg.errors.aadhaar_number);
                        $("#BiometricDataPid_errors").text('Need to scan before confirm');
                    } else if (msg.status == 'success') {
                        $("#add-sender-model").modal('hide');
                        if (msg.response_code != 1) {
                            $("#add-remitter-otp-model").modal('show');
                            $("#kyc_id").val(msg.kyc_id);
                            $("#ad1").val(msg.ad1);
                        } else {
                            getCustomer();
                        }
        ...
        function addBeneficiary() {
            ...
            var dataString = 'mobile_number=' + mobile_number + '&bank_id=' + bank_id + '&ifsc_code=' + ifsc_code + '&account_number=' + account_number + '&beneficiary_name=' + beneficiary_name + '&verify_status=' + verify_status + '&_token=' + token;
            $.ajax({
                type: "POST",
                url: "{{url('agent/money/v1/add-beneficiary')}}",
                data: dataString,
        ...
        function verifyRemitterOtp() {
            $("#remitterVerifyBtn").hide();
            $("#remitterVerifyBtn_loader").show();
            var token = $("input[name=_token]").val();
            var mobile_number = $("#mobile_number").val();
            ...
            var latitude = $("#inputLatitude").val();
            var longitude = $("#inputLongitude").val();
            var otp = $("#remitter_otp").val();
            var dataString = 'mobile_number=' + mobile_number + '&kyc_id=' + kyc_id + '&latitude=' + latitude + '&longitude=' + longitude + '&ad1=' + a1 + '&otp=' + otp + '&_token=' + token;
            $.ajax({
                type: "POST",
                url: "{{url('agent/money/v1/add-sender')}}",
                data: dataString,
                success: function (msg) {
                    ...
                    $("#beneficiaryVerifyBtn_loader").hide();
                    if (msg.status == 'error') {
                        $("#remitter_otp").text(msg.errors.otp);
                    } else if (msg.status == 'success') {
                        $("#add-remitter-otp-model").modal('hide');
                        swal("Success", msg.message, "success");
                        getCustomer();
                    } else {
                        swal("Failed", msg.message, "error");
        ...
        function transferNow() {
            ...
            if (latitude && longitude) {
                ...
                var transfer_otp = $("#transaction_otp").val();
                    ...
                    success: function (msg) {
                        ...
                        if (msg.status == 'success') {
                            if (msg.is_send_otp) {
                                ...
                                $('#transactionOtpDiv').removeClass('d-none');
                                $("#is_transfer_otp").val(2);
                                swal("Success", msg.message, "success");
                            } else {
                                $('#transactionDiv').removeClass('d-none');
                                $('#transactionOtpDiv').addClass('d-none');
            ...
            <div class="col-lg-4 col-md-12">
                <div class="card">
                                    ...
                                    <div class="input-group">
                                        ...
                                        {{-- <span class="input-group-btn">
                                             <button class="btn ripple btn-danger br-tl-0 br-bl-0" type="button"
                                                     onclick="accountVerify()">Verify</button>
                                         </span>--}}
                                    </div>
                                    <ul class="parsley-errors-list filled">
                    ...
                    <div class="modal-footer">
                        <button class="btn ripple btn-primary" type="button" onclick="addBeneficiary()"
                                id="addBeneficiaryBtn">Add Beneficiary
                        </button>
                    </div>
                </div>
            </div>
