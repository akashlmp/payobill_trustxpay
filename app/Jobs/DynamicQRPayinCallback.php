...
class DynamicQRPayinCallback implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        $input = [
            ...
            'secret_key' =>  $merchant['secrete_key'],
            'type' => 3,
            'mode' => 1
        ];
        ...
        try {
            ...
            \Log::info(['callback_error_' . $this->reference_id => $e->getMessage()]);
        }
    }
    ...
    public function getResponseArray($input)
    {
        ...
        $return_array['signature'] = hash('sha256', $input['customer_name'].$return_array['status'].number_format((float)$input['amount'], 2, '.', '').$input['customer_email'].$input['merchant_reference_id'].$input['secret_key']);
