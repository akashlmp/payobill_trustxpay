<?php

namespace App\Http\Controllers\Merchant;

use App\Http\Controllers\Controller;
use App\Library\BasicLibrary;
use App\Models\Loadcash;
use App\Models\MerchantUsers;
use App\Models\Paymentmethod;
use App\Models\WhiteListBank;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Auth;
use Helpers;
use DB;

class PayoutsRequestController extends Controller
{
    public function __construct()
    {

        $this->bank_name = '';
        $this->bank_account_name = '';
        $this->bank_ifsc = '';
        $this->company_id = Helpers::company_id()->id;
        $companies = Helpers::company_id();
        $this->cdn_link = $companies->cdn_link;

    }

    function paymentRequest(Request $request)
    {
        $usersData = Auth::guard('merchant')->user();
        $methods = Paymentmethod::where('status_id', 1)->get();
        $data = array('page_title' => 'Payment Request');
        $loadcash = Loadcash::where('merchant_id', $usersData->id)->get();
        $whiteListBanks = WhiteListBank::where('merchant_id', $usersData->id)->where('type', 2)->get();
        if (empty($usersData->account_number)) {
            $number = $usersData->id . 'M';
            $totalNumberLength = 12;
            $accountNumber = \Helpers::padWithZeros($number, $totalNumberLength);
            $accountNumber = 'PAYO' . $accountNumber;
            MerchantUsers::where('id', $usersData->id)->update(['account_number' => $accountNumber]);
            $usersData->account_number = $accountNumber;
        }
        $bank_ifsc = $this->bank_ifsc;
        $bank_name = $this->bank_name;
        $bank_account_name = $this->bank_account_name;
        return view('merchant.payment_request.index', compact('usersData', 'bank_account_name', 'bank_name', 'bank_ifsc', 'methods', 'loadcash', 'whiteListBanks'))->with($data);
    }

    public function addWhiteLabel()
    {
        return view('merchant.payment_request.white-label.add');
    }

    public function editWhiteLabel($id)
    {
        $data = WhiteListBank::where('id', $id)->first();
        return view('merchant.payment_request.white-label.edit', compact('data'));
    }

    public function storeWhiteLabel(Request $request)
    {
        $rules = array(
            'ifsc_code' => 'required',
            'account_number' => 'required',
            'payee_name' => 'required',
            'bank_name' => 'required',
            'bank_proof' => 'required|image|mimes:jpeg,png,jpg',
        );
        $validator = \Illuminate\Support\Facades\Validator::make($request->all(), $rules);
        if ($validator->fails()) {
            return Response()->json(['status' => 'validation_error', 'errors' => $validator->getMessageBag()->toArray()]);
        }
        try {
            $usersData = Auth::guard('merchant')->user();
            $record = WhiteListBank::where(['account_number' => $request->account_number, 'type' => 2])->first();
            if (!empty($record)) {
                return Response()->json(['status' => 'failure', 'message' => 'Account number already exist.']);
            } else {
                $path = "whitelist-bank";
                $image_url = Helpers::upload_s3_image($request->bank_proof, $path);
                WhiteListBank::create([
                    'merchant_id' => $usersData->id,
                    'ifsc_code' => $request->ifsc_code,
                    'account_number' => $request->account_number,
                    'type' => 2,
                    'payee_name' => $request->payee_name,
                    'bank_name' => $request->bank_name,
                    'bank_proof' => $this->cdn_link . $image_url,
                    'status' => 0, //Pending for approval
                ]);

                try {
                    $data = array(
                        'customer_name' => $usersData->first_name . ' ' . $usersData->last_name,
                        'subject' => 'Request to Review Bank Whitelist Addition',
                        'company_name' => 'Trustxpay',
                        'app_url' => url('/').'/admin/whitelist-merchant-bank-request',
                        'mobile' => $usersData->mobile_number,
                        'account_number' => $request->account_number,
                        'bank_name' => $request->bank_name,
                        'payee_name' => $request->payee_name,
                    );
                    \Mail::send('mail.merchant_add_bank', $data, function ($m) use ($usersData, $data) {
                        $m->to(["anil.mathukiya@payomatix.com","jatin.patel@payomatix.com","vaibhav.malhotra@payomatix.com"])->subject($data['subject']);
                        $m->from(env('MAIL_FROM_ADDRESS'), $data['company_name']);
                    });
                } catch (\Exception $e) {
                    \Log::error("Merchant add bank email send failed===" . $e->getMessage());
                }

                $userdetails = MerchantUsers::find($usersData->id);
                $letter = collect([
                    'title' => "Bank white list Notification",
                    'body' => "$userdetails->name $userdetails->last_name  white list bank added. please review.",
                ]);
                $parent_id = array(1);
                $library = new BasicLibrary();
                $library->send_notification($parent_id, $letter);
                return Response()->json(['status' => 'success', 'message' => 'Save successfully']);
            }
        } catch (\Exception $exception) {
            return Response()->json(['status' => 'failure', 'message' => $exception->getMessage()]);
        }
    }

    public function updateWhiteLabel(Request $request)
    {
        $rules = array(
            'ifsc_code' => 'required',
            'account_number' => 'required',
            'payee_name' => 'required',
            'bank_name' => 'required',
        );
        $validator = \Illuminate\Support\Facades\Validator::make($request->all(), $rules);
        if ($validator->fails()) {
            return Response()->json(['status' => 'validation_error', 'errors' => $validator->getMessageBag()->toArray()]);
        }
        try {
            $record = WhiteListBank::where(['account_number' => $request->account_number, 'type' => 2])->whereNot('id', $request->id)->first();
            if (!empty($record)) {
                return Response()->json(['status' => 'failure', 'message' => 'Account number already exist.']);
            } else {
                $usersData = Auth::guard('merchant')->user();
                $updateData = [
                    'ifsc_code' => $request->ifsc_code,
                    'account_number' => $request->account_number,
                    'payee_name' => $request->payee_name,
                    'bank_name' => $request->bank_name,
                    'status' => 0, //Pending for approval
                ];
                if (isset($request->bank_proof) && !empty($request->bank_proof)) {
                    $path = "whitelist-bank";
                    $imageUrl = Helpers::upload_s3_image($request->bank_proof, $path);
                    $updateData['bank_proof'] = $this->cdn_link . $imageUrl;
                }
                WhiteListBank::where('id', $request->id)->update($updateData);
                $userdetails = MerchantUsers::find($usersData->id);
                $letter = collect([
                    'title' => "Bank white list Notification",
                    'body' => "$userdetails->name $userdetails->last_name  updated information in white list bank. please review.",
                ]);
                $parent_id = array(1);
                $library = new BasicLibrary();
                $library->send_notification($parent_id, $letter);
                return Response()->json(['status' => 'success', 'message' => 'Updated successfully']);
            }
        } catch (\Exception $exception) {
            return Response()->json(['status' => 'failure', 'message' => $exception->getMessage()]);
        }
    }

    public function deleteWhiteLabel($id)
    {
        return view('merchant.payment_request.white-label.delete', compact('id'));
    }

    public function destroyBank(Request $request)
    {
        WhiteListBank::where('id', $request->id)->delete();
        return Response()->json(['status' => 'success', 'message' => 'Deleted successfully']);
    }
}
