...
class OrderController extends Controller
{
    ...
    function order_report (Request $request){
        if (Auth::User()->company->ecommerce == 1 && Auth::User()->profile->ecommerce == 1){
            if ($request->fromdate && $request->todate) {
                ...
                $urls = url('admin/ecommerce/order-report-api').'?'.'fromdate='.$fromdate.'&todate='.$todate.'&status_id='.$status_id;
            } else {
                $status_id = 4;
            ...
            return view('admin.ecommerce.my_orders', compact('delivery_statuses'))->with($data);
        }else{
            return redirect()->back();
        }
    }

    function order_report_api (Request $request){
        if (Auth::User()->company->ecommerce == 1 && Auth::User()->profile->ecommerce == 1){
    ...
    function view_track_order (Request $request){
        ...
        $ecommerce_productorders = EcommerceProductorder::where('id', $order_id)->first();
    ...
    function product_report (Request $request){
        if (Auth::User()->company->ecommerce == 1 && Auth::User()->profile->ecommerce == 1){
            if ($request->fromdate && $request->todate) {
                $fromdate = $request->fromdate;
                $todate = $request->todate;
                $status_id = $request->status_id;
                $urls = url('admin/ecommerce/product-report-api').'?'.'fromdate='.$fromdate.'&todate='.$todate.'&status_id='.$status_id;
            } else {
                $status_id = 4;
                $fromdate = date('Y-m-d', time());
                $todate = date('Y-m-d', time());
            ...
            $delivery_statuses = DeliveryStatus::get();
            return view('admin.ecommerce.product_report', compact('delivery_statuses'))->with($data);
        }else{
            return redirect()->back();
        }
    }

    function product_report_api (Request $request){
        if (Auth::User()->company->ecommerce == 1 && Auth::User()->profile->ecommerce == 1){
            $fromdate = $request->get('fromdate');
            $todate =  $request->get('amp;todate');
            $status_id =  $request->get('amp;status_id');
            ...
            foreach($records as $value){
                ...
                $data_arr[] = array(
                    ...
                    "username" => $value->user->name.' '.$value->user->last_name,
                    "created_at" => "$value->created_at",
                    "product" => '<div class="media"><div class="card-aside-img"><img src="'. $value->product->product_image.'" alt="img" class="h-60 w-60"></div><div class="media-body"> <div class="card-item-desc mt-0"> <h6 class="font-weight-semibold mt-0 text-uppercase">'.$value->subcategory->category_name.'</h6> <dl class="card-item-desc-1"> <dt>Name: </dt> <dd>'. $value->product->product_name.'</dd> </dl> <dl class="card-item-desc-1"> <dt>Weight: </dt> <dd>'. $value->product->product_weight.'</dd> </dl> </div> </div> </div>',
    ...
    function view_order_product_details (Request $request){
        ...
        $id = $request->id;
        $ecommerce_productorders = EcommerceProductorder::where('id', $id)->get();
        $productdetails = Self::get_product_details($ecommerce_productorders);

        //addresss details
        $ecommerce_productorders_ecommerceorder_id = EcommerceProductorder::where('id', $id)->first();
    ...
    function view_update_product (Request $request){
        if (Auth::User()->role_id == 1){
            $id = $request->id;
            $ecommerce_productorders = EcommerceProductorder::find($id);
            if ($ecommerce_productorders){
                $details = array(
    ...
    function track_order (Request $request){
        ...
        $ecommerce_productorders = EcommerceProductorder::where('ecommerceorder_id', $order_id)->get();
        return view('admin.ecommerce.track_order', compact('ecommerce_productorders'))->with($data);
    }
