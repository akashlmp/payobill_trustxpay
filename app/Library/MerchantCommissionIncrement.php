...
class MerchantCommissionIncrement
{
    ...
    {
        $apicommreports = MerchantApicommreport::where('report_id', $insert_id)->first();
        if ($apicommreports) {
            return Response()->json(['status' => 'failure', 'message' => 'already update!']);
        }
        $reports = MerchantTransactions::find($insert_id);
